...
class FeatureSelectionExperiment:
    def __init__(self, data_set_file, developers_dict_file, developers_list_file):
        ...
        logging.basicConfig(filename='feature_selection_experiment.log', \
    ...
    def _train_predict_cv(self):
        ...
        for key in self._rfe_cv:
            ...
            # since the epoch
            print_log(key)
            self._rfe_cv[key][-1] = self._rfe_cv[key][0](**self._rfe_cv[key][1]) \
            .fit(X_train, y_train)
...
def main():
    ...
    feature_selection_experiment = FeatureSelectionExperiment( \
